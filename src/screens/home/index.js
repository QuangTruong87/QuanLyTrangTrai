...
const HomeScreen = ({ route }) => {
    const { loginUsername } = route.params;

    const { user, logout} = useContext(AuthContext);
    const navigation = useNavigation();
    const isAdmin = user && user.viTriCongViec === 'admin';

    // Xử lý khi nhấn vào nút "Thêm nhân viên"
    const handleAdminButtonClick = () => {
        navigation.navigate('EmployeeList');
    };

    // Xử lý khi nhấn vào nút "Logout"
    const handleLogout = () => {
        logout();
    ...
    const [maTK, setMaTK] = useState(loginUsername);
...
<SafeAreaView style={styles.container}>
      ...
      <View style={styles.view_full_chucnang}>
        <View style={styles.view_chucnang}>
          <TouchableOpacity
          onPress={() => navigation.navigate('PetListScreen')}
          >
            <Image
        ...
        <View style={styles.view_chucnang}>
          <TouchableOpacity  onPress={() => navigation.navigate('Danh sách chu kỳ',{user:user})}>
            <Image
              style={styles.img_chucnang}
        ...
        <View style={styles.view_chucnang}>
          ...
          { isAdmin && (
          <TouchableOpacity style={styles.btn} onPress={handleAdminButtonClick}>
            <Image
              style={styles.img_chucnang}
            ...
            { isAdmin && ( <Text style={styles.txt_btnChuNang}>Nhân viên</Text>)}
        </View>
      ...
      <View style={styles.view_full_chucnang}>
       ...
       <View style={styles.view_chucnang}>
          <TouchableOpacity onPress={() => navigation.navigate('Danh sách khám bệnh',{user:user})}>
            <Image
              style={styles.img_chucnang}
              source={require('../../image/xembenh.png')}
              resizeMode="contain"
            />
          </TouchableOpacity>
          <Text style={styles.txt_btnChuNang}>Sức khỏe</Text>
       </View>
