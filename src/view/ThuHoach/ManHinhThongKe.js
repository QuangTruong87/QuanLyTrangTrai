...
export default function ManHinhThongKe({ route, navigation }) {
  ...
  const [chiPhiPhatSinh, setTongChiPhiPhatSinh] = useState(0);
  ...
  const getAPI = (search = '') => {
      fetch(`http://${ip}/API_QuanLyNongTrai/ChuKy/getDataLichChamNuoi.php?search=${search}`)
        ...
        .then(data => {
          ...
          setTongChiPhiPhatSinh(parseFloat(tongTienThuoc)+parseFloat(tongTienKham)+parseFloat(newItem.tienThietHai));
          setTienDauTuConDu(parseFloat(newItem.tienDauTu) - parseFloat(totalCost) - parseFloat(newItem.tienNhapGiong) - parseFloat(chiPhiPhatSinh) );


          setMaxCacLoaiTien(parseFloat(totalCost) + parseFloat(chiPhiPhatSinh)
   ...
   const [counter, setCounter] = useState(0);
       ...
       useEffect(() => {
           const maxRuns = 7; // Số lần tối đa cần chạy
           const intervalId = setInterval(() => {
               if (counter < maxRuns) {
                   getAPIKhamBenh(newItem.maCK);
                   getAPI(newItem.maCK);
           ...
           }, 100); // Thay đổi hình ảnh sau mỗi giây
  ...
  return (
    ...
    <SafeAreaView style={styles.container}>
      <Text style={styles.thongSo}>Thông tin chu kỳ</Text>
      <View style={styles.main_1}>
       <View style={styles.chiaDoi}>
        <View>
         <Image
              style={styles.hinhLuu}
              source={{ uri : newItem.hinh}}
            />
        </View>
      ...
      <View style={styles.main_2}>
          ...
          <View style={styles.chia2_2}>
           ...
           <View style={styles.chiaDoiCotTienThietHai}>
             ...
             <View style={styles.nho}>
              <Text style={styles.thuocTinh2}>Chi phí phát sinh {"\n"}(tiền thiệt hại + tiền khám {"\n"}+ tiền thuốc)</Text>
              <Text style={styles.thuocTinh2}>({parseFloat(chiPhiPhatSinh).toLocaleString('en-US')} VND)</Text>
             </View>
           </View>
          </View>
      ...
      <View style={styles.main_3}>
         ...
         <Image
          ...
          <View style={styles.chiaSoDoCuoi}>
            ...
            <View style={{ ...styles.progressBar,width: `${(chiPhiPhatSinh / maxCacLoaiTien) * 100}%`,
      ...
      <View style={styles.main_2}>
                ...
                <View style={styles.chia2_2}>
                 ...
                 <View style={styles.chiaDoiCotTienThietHai}>
                   <Text style={styles.cotTienThietHai}></Text>
                   <View style={styles.nho}>
                    <Text style={styles.thuocTinh2}>Chi phí phát sinh:</Text>
                    <Text style={styles.thuocTinh2}>({parseFloat( (chiPhiPhatSinh/maxCacLoaiTien)*100).toFixed(2) }% )</Text>
                   </View>
                 </View>
                </View>
