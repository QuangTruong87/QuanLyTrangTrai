...
export const AuthProvider = ({ children }) => {
    ...
    const getAPI = (search = '') => {
        fetch(`http://${ip}/API_QuanLyNongTrai/APINhanVien/getData.php?search=${search}`)
    ...
    const themNhanVien = async (nhanVien) => {
            try {
                const response = await fetch(`http://${ip}/API_QuanLyNongTrai/APINhanVien/addData.php`, {
                    method: 'POST',
                    headers: {
    ...
    const xoaNhanVien = async (tenDangNhap) => {
        try {
            const response = await fetch(`http://${ip}/API_QuanLyNongTrai/APINhanVien/delData.php`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
...
const suaNhanVien = async (thongTinMoi) => {
    try {
        ...
        const response = await fetch(`http://${ip}/API_QuanLyNongTrai/APINhanVien/editData.php`, {
            method: 'PUT',
            headers: {
                'Content-Type': 'application/json'
            },
