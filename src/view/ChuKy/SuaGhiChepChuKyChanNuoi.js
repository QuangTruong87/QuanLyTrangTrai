...
export default function SuaGhiChepChuKyChanNuoi({ route, navigation }) {
  ...
  const [tienDA, setTienDA] = React.useState(item.tienDoAn);
  const [luongDA, setLuongDA] = React.useState(item.khoiLuongAn);
  const [maChamSoc, setMaChamSoc] = React.useState(item.maChamSoc);
  const [ngayGhiChep, setNgayGhiChep] = React.useState(item.ngayChamSoc);
  const [DoAn_BS, setDoAn_BS] = React.useState(item.thucan_buoisang);
  const [BS, setBS] = React.useState('');
  const [Nuoc_BS, setNuoc_BS] = React.useState(item.nuoc_buoisang);

  const [DoAn_BTrua, setDoAn_BTrua] = React.useState(item.thucan_buoitrua);
  const [BT, setBT] = React.useState('');
  const [Nuoc_BTrua, setNuoc_BTrua] = React.useState(item.nuoc_buoitrua);

  const [DoAn_BToi, setDoAn_BToi] = React.useState(item.thucan_buoitoi);
  const [BToi, setBToi] = React.useState('');
  const [Nuoc_BToi, setNuoc_BToi] = React.useState(item.nuoc_buoitoi);
  ...
  const handleDelete = async () => {
      try {
          const response = await fetch(`http://${ip}/API_QuanLyNongTrai/ChuKy/delDataLichGhiChep.php`, {
    ...
    const handleEdit = async () => {
        try {
            const response = await fetch(`http://${ip}/API_QuanLyNongTrai/ChuKy/editDataLichGhiChep.php`, {
    ...
    <Modal
      ...
      <View style={styles.noiDungC}>
        ...
        <View style={styles.nhapDuLieu}>
          <Text style={styles.tieude_tb}>Mã chu kỳ: {item.maChuKy}</Text>
        ...
        <View style={styles.nhapDuLieu}>
          ...
          <Text style={styles.tieude_tb}>
            Số lượng nuôi: {item.soLuongNuoi} con
          </Text>
          {/* <TextInput style={styles.truongNhapDuLieu}></TextInput> */}
        </View>
        ...
        <View style={styles.thongTinDV}>
          ...
          <View style={styles.view_TT_DV}>
            ...
            <View>
              ...
              <Text style={styles.data_TT_DV}> Ngày nhận hàng: {item.ngayNhan}</Text>
      ...
      <View style={styles.view_xoa_sua}>
        <View style={{ width: '47%', marginRight: '6%' }}>
          <TouchableOpacity style={styles.btn_ThemChuKyChanNuoi} onPress={()=>handleYesNo('sua')}>
            <Text style={styles.txt_ChuKyChanNuoi}>Sửa </Text>
          </TouchableOpacity>
        </View>

        <View style={{ width: '47%' }}>
          <TouchableOpacity style={styles.btn_ThemChuKyChanNuoi}>
            <Text style={styles.txt_ChuKyChanNuoi} onPress={()=>handleYesNo('xoa')}>Xóa </Text>
          </TouchableOpacity>
        </View>
      </View>
    </SafeAreaView>
