...
export const AuthProvider2 = ({ children }) => {
    ...
    const AddPet = async (VatNuoi) => {
        try {
            const response = await fetch(`http://${ip}/API_QuanLyNongTrai/APIDongVat/addDongVat.php`, {
                method: 'POST',
                headers: {
    ...
    const removePet = async (maDV) => {
        try {
            const response = await fetch(`http://${ip}/API_QuanLyNongTrai/APIDongVat/delDongVat.php`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json',
    ...
    const updatePet = async (thongTinMoi) => {
        try {
            ...
            const response = await fetch(`http://${ip}/API_QuanLyNongTrai/APIDongVat/editDongVat.php`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json'
                },
